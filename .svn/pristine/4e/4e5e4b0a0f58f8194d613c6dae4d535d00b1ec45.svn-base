...
class QuizController extends CommonController
{
	...
	public function actionHtmlIntro()
	{
		...
		else{echo "<p>Student Attempt Limit: <b>$allowedattempt</b></p>";}
	...
	public function actionHtmlQuestion()
	{
		...
		if($question->answertype == CMDB_QUIZQUESTION_LONGTEXT)
		{
			$attemptid = getparam('attemptid');
			$answer = getdbosql('QuizAttemptAnswer', "attemptid=$attemptid and questionid=$question->id");

			echo "<br><textarea name='quiz_answer_longtext' id='quiz_answer_longtext'>$answer->answerlong</textarea>";
			showHtmlEditor('quiz_answer_longtext', 300, 'custom1');

			echo <<<end
			<table><tr><td>
<span style="display: inline-block; margin-top: 10px; width: 140px; height: 40px; overflow: hidden;">
	<button id='button_upload' style="position: relative;">
		<a href="#">Add File</a>
	</button>
	<input multiple="multiple" type="file" id="upload_field" 
		style="font-size: 40px; width: 100px; opacity: 0; 
		filter:alpha(opacity: 0);  position: relative; top: -40px; left: -20px" />
</span>
</td><td>
<span id="progress_report" style="display: inline-block; margin-top: 0; border: solid 1px #DDF0DD; background-color: #EBFFEB; padding: 8px;">
	<span id="progress_report_name"></span>
	<span id="progress_report_bar_container" style="width: 100%; height: 3px;">
		<span id="progress_report_bar" style="background-color: blue; width: 0; height: 100%;"></span>
	</span>
</span>
</td></tr></table>
					
<script>
	initializeHtml5Upload();
</script>
end;
	...
	//	$quiz = getdbo('Quiz', "quizid=$attempt->id");
		...
		switch($question->answertype)
		{
			case CMDB_QUIZQUESTION_LONGTEXT:
				...
				$fileid = HandleUploadedFiles($parent->id);
				if($fileid) $answer->answerfileid = $fileid;
			...
			case CMDB_QUIZQUESTION_SELECT:
				...
				$answer->answerselectid = getparam('answerselectid');
				$answer->save();
			...
			case CMDB_QUIZQUESTION_COMPARATIVE:
				...
				$inname = SANSSPACE_TEMP."/phpsessid=$phpsessid&questionid=$question->id.flv";
				...
				if(file_exists($inname))
				{
					if(!$answer->answerfile)
					{
						...
						$object->name = "attempt=$attempt->id,question=$question->id";
				...
				//	debuglog("rename($inname, $filename)");
					@rename($inname, $filename);
					@unlink($inname.FLV_INDEX_EXTENSION2);
